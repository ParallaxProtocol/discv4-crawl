...
update_devp2p_tool() {
  git_update_repo https://github.com/ParallaxProtocol/parallax "$parallax_src"
  (cd "$parallax_src" && go build ./cmd/devp2p)
}
...
generate_list() {
  if [ -n "$CRAWL_BOOTNODES" ]; then
    devp2p discv4 crawl --parallel 4 --timeout "${CRAWL_TIMEOUT}" --bootnodes "$CRAWL_BOOTNODES" --addr "${CRAWL_LISTEN_ADDR}" all.json
  else
    devp2p discv4 crawl --parallel 4 --timeout "${CRAWL_TIMEOUT}" --addr "${CRAWL_LISTEN_ADDR}" all.json
  fi
...
PATH="$parallax_src:$PATH"
cd output

init_prometheus_metrics

while true; do
  # Pull changes from parallax core client
  update_devp2p_tool
