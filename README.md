...
[Parallax Core](https://github.com/ParallaxProtocol/parallax) ships with an implementation of [EIP-1459](https://eips.ethereum.org/EIPS/eip-1459). This EIP defines a way to put devp2p node lists behind a DNS name. There are a couple of things worth knowing about this system:

    EIP-1459 is intended to be a replacement for hard-coded bootstrap node lists that we maintain in Parallax clients.
    ...
    The node list is signed with a key which will be hard-coded into the client (i.e. prlx) and which we should keep in a secure place.
...
You can read the [DNS Discovery Setup Guide](https://docs.parallaxprotocol.org/parallax-client/developers/client/dns-disc-guide) for more information about the discovery DHT crawler.
...
-----| ------- | -------
`CRAWL_GIT_REPO` | `https://github.com/ParallaxProtocol/discv4-dns-lists.git` | Git repository `used to clone and push the node list
`CRAWL_GIT_BRANCH` | `main` | Git branch used for the fetch and push
...
$ docker run -it \
    ...
    -e CRAWL_GIT_REPO=git@github.com:ParallaxProtocol/discv4-dns-lists.git \ # Use SSH instead of HTTPS
    -e CRAWL_GIT_PUSH=true \ # Specify that we want to push the changes
    ParallaxProtocol/discv4-crawl
```
